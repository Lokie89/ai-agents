...
6. 로컬 하네스가 없거나 부족하면 `AskUserQuestion`으로 필요한 질문을 먼저 제시한다.
...
11. 청사진, 계획, 설계만 요청받았다면 `EnterPlanMode`로 전환해 실행 없이 계획을 먼저 제시한다.
12. 사용자가 실행을 승인했거나 명확한 구현 요청이면 저장소 구조를 확인한다.
13. `Glob`/`Grep`으로 관련 파일을 검색한다.
14. 기존 구현 패턴과 테스트 방식을 `Read`로 확인한다.
15. 필요한 변경만 `Edit`/`Write`로 적용한다.
