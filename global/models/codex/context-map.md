...
4. 기술 스택, 배포 구조, 서버 구성, 통신 방식, 데이터 저장소 같은 아키텍처 전제가 필요한지 확인한다.
5. 프로젝트별 로컬 하네스가 필요한지 `project-rules.md` 기준으로 판단한다.
6. 로컬 하네스가 없거나 부족하면 필요한 질문을 먼저 제시한다.
7. 개념 추가가 있으면 도메인 정의와 정책이 명확한지 확인한다.
8. 아키텍처 전제가 구현에 영향을 주는데 `local/<project-name>/architecture.md`가 없거나 부족하면 청사진 질문을 먼저 제시한다.
9. 도메인 정책이 불명확하면 구현 전에 질의를 통해 정책을 먼저 세운다.
10. 청사진만 필요한 요청인지 확인한다.
11. 청사진, 계획, 설계만 요청받았다면 읽기 전용 조사로 근거를 확보한 뒤 변경 없이 계획을 제시한다.
12. 사용자가 실행을 승인했거나 명확한 구현 요청이면 저장소 구조를 확인한다.
13. 관련 파일을 검색한다.
14. 기존 구현 패턴과 테스트 방식을 읽는다.
15. 필요한 변경만 적용한다.
16. 검증을 실행하고 결과를 기록한다.
...
- 구현 전제에 영향을 주는 기술 스택, 런타임, PC/서버 스펙, 배포 단위, 단일 서버/클러스터 여부, BFF/API Gateway, 통신 방식, 데이터 저장소, 캐시, 메시지 큐, 이벤트 브로커, 인증/권한, 운영 요구가 불명확하면 질문한다.
...
- 작업 대상 하위 트리에 `AGENTS.md`가 있으면 루트 지침에 더해 해당 하위 지침을 확인한다.
- 로컬 규칙이 필요한데 없으면 `project-rules.md`의 로컬 하네스 대화 기준을 따른다.
- 파일 검색은 우선 `rg` 또는 `rg --files`를 사용한다.
- 넓은 탐색은 필요한 질문 중심으로 범위를 제한하고, 원시 로그나 대량 출력보다 관련 파일과 결론을 요약해 메인 컨텍스트를 작게 유지한다.
...
- 외부 최신 정보가 필요한 경우 공식 문서나 1차 출처를 우선 확인하고, 사용한 출처를 결과에 남긴다.
